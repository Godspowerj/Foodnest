...
const CategoryMenu = () => {
  const { filteredData, selectCategory, addToCart, updateCartQuantity, cart,handleFavourite } =
        ...
        (filteredData as Products[]).map((item, index) => (
          ...
          >
            <div className="relative sm:flex flex-col items-center space-y-3">
              ...
              />
              <span className="absolute left-0 top-2 -translate-y-1/2 text-[#6e4231] w-3 h-3 " onClick={()=>handleFavourite(item) }>
                <svg
